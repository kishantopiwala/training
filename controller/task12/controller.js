...
async function submitform(req, res) {
    ...
    function education_details(req, res, lastindex) {
    ...
    function work_details(req, res, lastindex) {
    ...
    function reference_contact(req, res, lastindex) {
    ...
    function preference(req, res, lastindex) {
        let { p_location, notice_period, expacted_ctc, current_ctc, p_department } = req.body
        var insert_reference = `insert into preferences(emp_id,pref_city,expected_ctc,current_ctc,notice_period,pref_department) value(${lastindex},'${p_location}' ,${expacted_ctc},${current_ctc},${notice_period},'${p_department}')`;
        con.query(insert_reference, (error) => {
            if (error) {
                console.log(error)
                res.send("Error in inserting Data")
    ...
    function language(req, res, lastindex) {
    ...
    function technology(req, res, lastindex) {
    ...
    basic_details().then((lastindex) => {
        console.log(lastindex)
        education_details(req, res, lastindex)
        work_details(req, res, lastindex)
        reference_contact(req, res, lastindex)
        preference(req, res, lastindex);
        language(req, res, lastindex)
        technology(req, res, lastindex)
...
async function updateemployee(req, res) {
    ...
    function updatework(req, res) {
        ...
        return new Promise((resolve, reject) => {
            workids = workid.filter((workid) => workid.length > 1)
            companys = companys.filter((company) => company.length > 1)
            workdesignation = workdesignation.filter((des) => des.length > 1)
            fromdates = fromdates.filter((frmd) => frmd.length == 10)
            todates = todates.filter((tod) => tod.length == 10)
            ...
            for (i = 0; i < companys.length; i++) {
                var update_work_details = `update work_experiance set company_name = ? ,designation = ?,from_date = ?,to_date = ? where id =${workids[i]} and emp_id= ${emp_id};`
...
module.exports = { jobform, submitform, showemployees, getemployeedetail, updateemployee, renderemployee }